...
class IADDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _,seg_b1 = cv2.threshold(label1,0,1,cv2.THRESH_BINARY_INV)
        _,seg_b2 = cv2.threshold(label2,0,1,cv2.THRESH_BINARY_INV)
        _,seg1 = cv2.threshold(label1,0,1,cv2.THRESH_BINARY)
        _,seg2 = cv2.threshold(label2,0,1,cv2.THRESH_BINARY)
